...
class T8N(Load):
    ...
    alloc: Any
    senders_authorities: Dict[Uint, Set[Any]]
    ...
    def __init__(
        ...
        *,
        ...
        # Monad tracks, per block number, which accounts sent a
        # transaction or were an authority. The fork's state carries it
        # so the reserve-balance rules can consult recent history.
        self.senders_authorities = {
            Uint(int(block_number)): {
                self.fork.hex_to_address("0x" + bytes(address).hex())
                for address in addresses
            }
            for block_number, addresses in t8n_data.senders_authorities.items()
        }
    ...
    def block_environment(self) -> Any:
        ...
        if self.fork.has_senders_authorities:
            block_env.state._senders_authorities = self.senders_authorities
    ...
    def run(self) -> "TransitionToolOutput":
        ...
        """
        from execution_testing.base_types import Address as TestingAddress
        ...
        return TransitionToolOutput(
            ...
            senders_authorities={
                int(block_number): [
                    TestingAddress(bytes(address)) for address in addresses
                ]
                for block_number, addresses in self.senders_authorities.items()
            },
            result=self.result,
