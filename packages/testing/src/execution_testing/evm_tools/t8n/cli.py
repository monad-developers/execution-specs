...
def build_t8n_from_cli_options(
    ...
    raw_senders_authorities_json = _read_json_input(
        options.input_senders_authorities, stdin, "senders_authorities"
    )
    ...
    t8n_data = TransitionTool.TransitionToolData(
        alloc=alloc,
        senders_authorities={
            int(block_number): [Address(a) for a in addresses]
            for block_number, addresses in raw_senders_authorities_json.items()
        },
        env=env,
        txs=txs,
        fork=testing_fork,
...
def write_t8n_outputs(
    ...
    json_senders_authorities = {
        str(block_number): ["0x" + bytes(a).hex() for a in addresses]
        for block_number, addresses in output.senders_authorities.items()
    }
    if options.output_senders_authorities == "stdout":
        json_output["senders_authorities"] = json_senders_authorities
    else:
        senders_authorities_output_path = os.path.join(
            options.output_basedir, options.output_senders_authorities
        )
        with open(senders_authorities_output_path, "w") as f:
            json.dump(json_senders_authorities, f, indent=4)
        t8n.logger.info(
            f"Wrote senders authorities to {senders_authorities_output_path}"
        )
...
def _clean_output_dir(options: Any) -> None:
    ...
    files_to_delete = [
        ...
        options.output_senders_authorities,
        options.output_body,
    ]
