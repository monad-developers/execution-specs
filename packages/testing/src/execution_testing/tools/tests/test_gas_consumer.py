...
def test_gas_consumer_exact_cost(fork: Fork, gas: int) -> None:
    ...
    # Under Monad's memory cap the larger targets fall to `JUMPDEST`,
    # which its code size cannot hold.
    if fork.max_tx_memory_usage() is not None and gas > 262_144:
        with pytest.raises(ValueError, match="over its"):
            GasConsumer(gas=gas, fork=fork)
        return
