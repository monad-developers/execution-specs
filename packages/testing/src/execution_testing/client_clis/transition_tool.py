...
class TransitionTool(EthereumCLI):
    ...
    def _evaluate_filesystem(
        ...
        *,
        ...
        output_paths = {
            ...
            for output in ["alloc", "result", "senders_authorities"]
        }
    ...
    def safe_t8n_args(
        ...
        *,
        ...
        # env/txs/senders_authorities still come through stdin; alloc is
        # either bundled on stdin (initial block) or streamed from a path
        # (chained-block handoff). Outputs always go to files in the
        # existing temp dir so Python can stream-read the post-state alloc
        # rather than buffering stdout.
        input_alloc = (
        ...
        output_senders_authorities: LiteralString = (
            "--output.senders_authorities=output/senders_authorities.json"
        )
