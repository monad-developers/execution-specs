...
def test_cost_non_quadratic(
    ...
    tx = Transaction(
        gas_limit=fork.gas_costs().MEMORY_PER_WORD
...
def memory_copy_opcodes(
    ...
    memory_opcodes = {
        Op.CALLDATACOPY: gas_costs.VERY_LOW,
        Op.CODECOPY: gas_costs.VERY_LOW,
        Op.EXTCODECOPY: gas_costs.WARM_ACCESS,
        Op.MCOPY: gas_costs.VERY_LOW,
        Op.SHA3: gas_costs.OPCODE_KECCAK256_BASE,
        Op.LOG0: gas_costs.OPCODE_LOG_BASE,
        Op.LOG1: gas_costs.OPCODE_LOG_BASE + gas_costs.OPCODE_LOG_TOPIC,
        Op.LOG2: gas_costs.OPCODE_LOG_BASE + 2 * gas_costs.OPCODE_LOG_TOPIC,
        Op.LOG3: gas_costs.OPCODE_LOG_BASE + 3 * gas_costs.OPCODE_LOG_TOPIC,
        Op.LOG4: gas_costs.OPCODE_LOG_BASE + 4 * gas_costs.OPCODE_LOG_TOPIC,
        Op.RETURN: 0,
        Op.REVERT: 0,
        Op.CREATE: gas_costs.OPCODE_CREATE_BASE,
        Op.CREATE2: gas_costs.OPCODE_CREATE_BASE,
        Op.CALL: gas_costs.WARM_ACCESS,
        Op.DELEGATECALL: gas_costs.WARM_ACCESS,
        Op.STATICCALL: gas_costs.WARM_ACCESS,
        Op.CALLCODE: gas_costs.WARM_ACCESS,
...
def memory_stack_opcodes(
    ...
    memory_opcodes = {
        Op.MLOAD: gas_costs.VERY_LOW,
        Op.MSTORE: gas_costs.VERY_LOW,
        Op.MSTORE8: gas_costs.VERY_LOW,
    }
...
def test_memory_copy_opcodes(
    ...
    if opcode in (
        ...
        dynamic_gas_cost = fork.gas_costs().OPCODE_COPY_PER_WORD * (
            (size + 31) // 32
        )
    if opcode == Op.SHA3:
        dynamic_gas_cost = fork.gas_costs().OPCODE_KECCAK256_PER_WORD * (
            (size + 31) // 32
        )
    if opcode in (Op.LOG0, Op.LOG1, Op.LOG2, Op.LOG3, Op.LOG4):
        dynamic_gas_cost = fork.gas_costs().OPCODE_LOG_DATA_PER_BYTE * size
    if opcode in (Op.RETURN, Op.REVERT):
        dynamic_gas_cost = 0
    if opcode in (Op.CREATE, Op.CREATE2):
        init_code_cost = fork.gas_costs().CODE_INIT_PER_WORD * (
            (size + 31) // 32
        )
        if opcode == Op.CREATE2:
            hash_cost = fork.gas_costs().OPCODE_KECCAK256_PER_WORD * (
                (size + 31) // 32
            )
...
def test_returndatacopy_gas_cost(
    ...
    dynamic_gas_cost = gas_costs.OPCODE_COPY_PER_WORD * ((size + 31) // 32)
    base_gas = gas_costs.VERY_LOW
...
def test_consecutive_expansions(
    ...
    """
    gas_costs = fork.gas_costs()
    base_gas_per_op = gas_costs.VERY_LOW
