...
Ported from:
tests/static/state_tests/VMTests/vmLogTest/log2Filler.yml
"""
...
def test_log2(
    ...
    pre.deploy_contract(
        code=(
            ...
            + Op.LOG2(
                offset=Op.SUB(0x0, 0x1), size=0x1, topic_1=0x0, topic_2=0x0
            )
            + Op.SSTORE(key=0x0, value=0x600D)
            + Op.STOP
        ),
    ...
    pre.deploy_contract(
        code=(
            ...
            + Op.LOG2(
                offset=0x1, size=Op.SUB(0x0, 0x1), topic_1=0x0, topic_2=0x0
            )
            + Op.SSTORE(key=0x0, value=0x600D)
            + Op.STOP
        ),
