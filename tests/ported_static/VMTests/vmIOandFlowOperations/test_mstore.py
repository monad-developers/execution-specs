...
Ported from:
tests/static/state_tests/VMTests/vmIOandFlowOperations/mstoreFiller.yml
"""
...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/VMTests/vmIOandFlowOperations/mstoreFiller.yml",  # noqa: E501
    ],
)
...
def test_mstore(
    ...
    pre.deploy_contract(
        code=(
            Op.MSTORE(offset=0x1, value=Op.SUB(0x0, 0x1))
            + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x1))
            + Op.STOP
        ),
        balance=0xBA1A9CE0BA1A9CE,
    ...
    pre.deploy_contract(
        code=(
            Op.MSTORE(offset=0x1, value=0xFF)
    ...
    pre.deploy_contract(
        code=(
            Op.MSTORE(offset=0x1, value=0xFF00)
            + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0))
            + Op.STOP
        ),
        balance=0xBA1A9CE0BA1A9CE,
