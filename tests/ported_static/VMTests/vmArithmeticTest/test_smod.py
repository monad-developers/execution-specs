...
Ported from:
tests/static/state_tests/VMTests/vmArithmeticTest/smodFiller.yml
"""
...
def test_smod(
    ...
    pre.deploy_contract(
        code=(
            Op.SSTORE(key=0x0, value=Op.SMOD(Op.SUB(0x0, 0x2), 0x3)) + Op.STOP
        ),
        balance=0xBA1A9CE0BA1A9CE,
        nonce=0,
    ...
    pre.deploy_contract(
        code=(
            Op.SSTORE(key=0x0, value=Op.SUB(Op.SMOD(0x10, 0x0), 0x1)) + Op.STOP
        ),
        balance=0xBA1A9CE0BA1A9CE,
        nonce=0,
