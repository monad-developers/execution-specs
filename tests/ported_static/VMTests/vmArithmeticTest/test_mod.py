...
Ported from:
tests/static/state_tests/VMTests/vmArithmeticTest/modFiller.yml
"""
...
def test_mod(
    ...
    pre.deploy_contract(
        code=Op.SSTORE(key=0x0, value=Op.MOD(0x2, 0x3)) + Op.STOP,
    ...
    pre.deploy_contract(
        code=Op.SSTORE(key=0x0, value=Op.MOD(0x3, 0x0)) + Op.STOP,
        balance=0xBA1A9CE0BA1A9CE,
        nonce=0,
        address=Address("0x0000000000000000000000000000000000001003"),  # noqa: E501
    )
    pre.deploy_contract(
        code=Op.SSTORE(key=0x0, value=Op.MOD(Op.SUB(0x0, 0x2), 0x3)) + Op.STOP,
        balance=0xBA1A9CE0BA1A9CE,
        nonce=0,
    ...
    pre.deploy_contract(
        code=(
            Op.SSTORE(key=0x0, value=Op.SUB(Op.MOD(0x10, 0x0), 0x1)) + Op.STOP
        ),
        balance=0xBA1A9CE0BA1A9CE,
        nonce=0,
