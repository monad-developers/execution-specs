...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/stEIP150singleCodeGasPrices/RawCallMemoryGasAskFiller.json",  # noqa: E501
    ],
)
@pytest.mark.valid_from("Cancun")
@pytest.mark.pre_alloc_mutable
def test_raw_call_memory_gas_ask(
    ...
    if fork >= MONAD_NINE:
        gas_adj -= 747
    # _ask gives all gas to callee; on M9 the callee gets less.
    callee_adj = 6648 if fork >= MONAD_NINE else 0

    post = {
        contract: Account(storage={1: 25608 + gas_adj}),
        callee: Account(storage={2: 0x72464 - callee_adj}),
    }
