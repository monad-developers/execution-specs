"""
Paged-storage implementation of the shared state model (MIP-8).

The [`State`] class here mirrors [`ethereum.state_mpt.State`], except that
each account's storage is committed with the MIP-8 page-based scheme
instead of a Merkle Patricia Trie. Accounts still live in a keccak256 MPT,
so the state root differs from the mainnet one only in the storage roots it
embeds.

[`State`]: ref:ethereum.state_paged.State
[`ethereum.state_mpt.State`]: ref:ethereum.state_mpt.State
"""
...
class State:
    ...
    def compute_state_root(self, block_diff: BlockDiff) -> Root:
        ...
        The diff's ``code_changes`` play no part: the account trie commits
        to accounts' code hashes, never to code contents, so account diffs
        alone determine the root.
