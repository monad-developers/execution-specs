...
from typing import Dict, Optional, Set, Tuple, final
...
def process_message(message: Message) -> Evm:
    ...
    # MIP-8: child frames inherit the parent's page warming and per-page
    # growth; the top-level frame seeds its read-warm pages from the
    # transaction access list (a warmed slot warms its whole page).
    init_read_pages: Set[Tuple[Address, U256]] = set()
    init_write_pages: Set[Tuple[Address, U256]] = set()
    init_current_growth: Dict[Tuple[Address, U256], int] = {}
    init_net_growth: Dict[Tuple[Address, U256], int] = {}

    if message.parent_evm is not None:
        init_read_pages = message.parent_evm.read_accessed_pages.copy()
        init_write_pages = message.parent_evm.write_accessed_pages.copy()
        init_current_growth = dict(message.parent_evm.current_state_growth)
        init_net_growth = dict(message.parent_evm.net_state_growth)
    else:
        for addr, slot_key in message.accessed_storage_keys:
            slot_val = U256.from_be_bytes(slot_key)
            init_read_pages.add((addr, page_index(slot_val)))
    ...
    evm = Evm(
        ...
        read_accessed_pages=init_read_pages,
        write_accessed_pages=init_write_pages,
        current_state_growth=init_current_growth,
        net_state_growth=init_net_growth,
    )
