...
# Monad-specific: reserve balance exception window.
...
def add_sender_authority(
    ...
    """
    Record `address` as a sender or authority in the given block number.

    Used by the reserve balance "emptying transaction" exception: an
    undelegated EOA may go below RESERVE_BALANCE only if neither it nor
    a delegation/undelegation touching it appeared in the previous
    RESERVE_BALANCE_DELAY_BLOCKS blocks.
    """
    block_state._senders_authorities.setdefault(block_number, set()).add(
        address
    )
...
def is_sender_authority(
    ...
    """
    Return True if `address` was a sender or authority in any of the
    block numbers currently retained on `block_state` (the previous
    RESERVE_BALANCE_DELAY_BLOCKS blocks).
    """
    for (
        senders_authorities_at_block
    ) in block_state._senders_authorities.values():
...
def forget_senders_authorities(
    ...
    """
    Prune senders/authorities older than the reserve balance delay window.
    """
    if current_block_number >= RESERVE_BALANCE_DELAY_BLOCKS:
        number = current_block_number - RESERVE_BALANCE_DELAY_BLOCKS
        block_state._senders_authorities.pop(number, None)


def get_balance_original(snapshot: TransactionState, address: Address) -> U256:
    """
    Get the balance of `address` as it was at the start of the
    top-level EVM execution (i.e. after pre-execution gas/nonce
    deduction but before any EVM-driven writes).

    Reads from a ``TransactionState`` captured at top-level EVM begin
    (the same ``snapshot`` used for rollback in the interpreter). Read
    chain: snapshot.account_writes -> block writes -> pre_state.
    """
    account = get_account_optional(snapshot, address)
    if account is None:
        return U256(0)
    return account.balance


def iter_all_addresses(tx_state: TransactionState) -> Set[Address]:
    """
    Return every address touched by pre-state, block, or current tx.

    Union of pre-state addresses, block-level writes, and current-tx
    writes. Reproduces the old ``state._main_trie._data.keys()`` view
    used by Monad's end-of-tx reserve-balance sweep. Assumes the
    concrete ``State`` from ``ethereum.state`` (t8n loads its alloc
    into one) so ``_main_trie._data`` is enumerable.
    """
    pre_state = tx_state.parent.pre_state
    pre_addrs: Set[Address]
    if hasattr(pre_state, "_main_trie"):
        pre_addrs = set(pre_state._main_trie._data.keys())
    else:
        pre_addrs = set()
    return (
        pre_addrs
        | set(tx_state.parent.account_writes.keys())
        | set(tx_state.account_writes.keys())
    )
